...
public sealed class DeviceIdentityService : IDeviceIdentityService
{
    ...
    private DeviceIdentity GetOrCreateSynchronized(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (identityExists)
            {
                DeviceIdentity identity = LoadAndValidateAsync(cancellationToken).GetAwaiter().GetResult();
                _logger.LogInformation("Device identity loaded ({DeviceId}).", Mask(identity.DeviceId));
                return identity;
            }

            DeviceIdentity created = CreateAsync(cancellationToken).GetAwaiter().GetResult();
    ...
    private async Task<DeviceIdentity> LoadAndValidateAsync(CancellationToken cancellationToken)
    {
