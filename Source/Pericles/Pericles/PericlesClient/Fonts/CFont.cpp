...
auto CFont::InitFont( IFW1Factory* pFW1Factory , std::wstring Name , float FontSize ) -> void
{
	...
	if ( FAILED( pFW1Factory->CreateFontWrapper( GetPericlesGUI()->GetDevice() , NULL , &createParams , &m_pFontWrapper ) ) )
	{
...
auto CFont::DrawString( int x , int y , ImColor Color , int Flags , const char* fmt , ... ) -> void
{
	...
	if ( m_pFontWrapper )
	{
		...
		ID3D11DeviceContext* pContext = GetPericlesGUI()->GetDeviceContext();
		...
		GetPericlesGUI()->GetDevice()->CreateBlendState( &blendDesc , &pBlendState );
		...
		{
			m_pFontWrapper->DrawString( GetPericlesGUI()->GetDeviceContext() ,
