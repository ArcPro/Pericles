...
auto CL_Trace::IsEntityVisibleAtPoint( const Vector3& CameraPos , const Vector3& EndPos , C_BaseEntity* pTargetEntity ) -> bool
{
	if ( !pTargetEntity )
		return false;

	auto* pLocalPlayerPawn = GetCL_CitadelPlayerPawn()->GetLocal();
	if ( !pLocalPlayerPawn )
		return false;
	...
	CGameTrace GameTrace{};
	CTraceFilter Filter( 0x1C1003 , pLocalPlayerPawn , 3 , 15 );
	auto Start = CameraPos;
	auto End = EndPos;

	if ( !CVPhys2World_TraceShape( SDK::Pointers::GetCVPhys2World() , &Ray , &Start , &End , &Filter , &GameTrace ) )
		return true;

	return GameTrace.pHitEntity == pTargetEntity || !GameTrace.DidHit();
}
