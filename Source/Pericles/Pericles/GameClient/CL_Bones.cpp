...
{
	if ( !PrepareEntityBones( pC_BaseEntity ) )
		return {};

	return GetPreparedBonePositionByName( pC_BaseEntity , szBoneName );
}

auto CL_Bones::PrepareEntityBones( C_BaseEntity* pC_BaseEntity ) -> bool
{
	if ( !pC_BaseEntity )
		return false;

	auto* pSceneNode = pC_BaseEntity->m_pGameSceneNode();
	if ( !pSceneNode )
		return false;
	...
	return pSkeletonInstance->m_modelState().m_hModel().is_valid();
}

auto CL_Bones::GetPreparedBonePositionByName( C_BaseEntity* pC_BaseEntity , const char* szBoneName ) -> Vector3
{
	Vector3 BonePosition;
	if ( !pC_BaseEntity || !szBoneName )
		return BonePosition;

	auto* pSceneNode = pC_BaseEntity->m_pGameSceneNode();
	auto* pSkeletonInstance = pSceneNode ? pSceneNode->GetSkeletonInstance() : nullptr;
	if ( !pSkeletonInstance || !pSkeletonInstance->m_modelState().m_hModel().is_valid() )
		return BonePosition;
