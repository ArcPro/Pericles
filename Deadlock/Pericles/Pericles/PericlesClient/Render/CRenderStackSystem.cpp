...
auto CRenderObjectDrawLineBatch::OnRender() -> void
{
	for ( const auto& Line : m_Lines )
		GetRender()->DrawLine( Line.Start , Line.End , Line.Color , Line.Thickness );
}
...
auto CRenderStackSystem::DrawLineBatch( std::vector<RenderLine_t>&& Lines ) -> void
{
	if ( Lines.empty() )
		return;

	std::scoped_lock lock( m_Lock );
	m_vecUpdateBuffer.emplace_back( std::make_shared<CRenderObjectDrawLineBatch>( std::move( Lines ) ) );
}
...
auto CRenderStackSystem::OnClientOutput() -> void
{
	...
	m_vecUpdateBuffer.reserve( m_pSharedBuffer.load()->size() );
	m_bSharedBufferReady = true;
}
